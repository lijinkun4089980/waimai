...
public abstract class ThreadLocalWrapper {
     ...
     * 分页总条数
     */
    private static ThreadLocal<Integer> rowSize = new ThreadLocal<Integer>();

    public static void setRowSize(Integer _rowSize) {
        rowSize.set(_rowSize);
    }

    public static Integer getRowSize() {
        Integer _rowSize = (Integer) rowSize.get();
        return _rowSize;
    }

    public static void removeRowSize() {
        rowSize.remove();
    }
}
